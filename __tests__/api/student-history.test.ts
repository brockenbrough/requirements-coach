...
import { GET } from '../../app/api/students/[studentId]/history/route';

function makeRequest(token = 'valid-token') {
  return new Request('http://localhost/api/students/user-123/history', {
    headers: { Authorization: `Bearer ${token}` },
  });
}

const PARAMS = (studentId: string) => ({ params: { studentId } });
...
describe('GET /api/students/:studentId/history', () => {
  it('returns 401 without a token', async () => {
    const res = await GET(new Request('http://localhost/api/students/user-123/history'), PARAMS('user-123'));
    expect(res.status).toBe(401);
  });

  it('returns 401 for an invalid token', async () => {
    const res = await GET(makeRequest('bad-token'), PARAMS('user-123'));
    expect(res.status).toBe(401);
  });

  it('returns 403 when studentId does not match the authenticated user', async () => {
    const res = await GET(makeRequest(), PARAMS('other-user-id'));
    expect(res.status).toBe(403);
  });

  it('returns an empty array when no completed sessions exist', async () => {
    queue('session_log', { data: [], error: null });
    const res = await GET(makeRequest(), PARAMS('user-123'));
  ...
  it('only returns completed sessions', async () => {
    queue('session_log', { data: [], error: null });
    await GET(makeRequest(), PARAMS('user-123'));
