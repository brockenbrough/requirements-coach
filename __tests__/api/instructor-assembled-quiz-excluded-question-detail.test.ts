...
import { DELETE } from '../../app/api/instructor/assembled-quizzes/[quizId]/excluded-questions/[questionId]/route';
...
function delReq(quizId = 'quiz-1', questionId = 'q-1', token: string | null = 'valid-token') {
  return DELETE(
    new Request(`http://localhost/api/instructor/assembled-quizzes/${quizId}/excluded-questions/${questionId}`, {
      method: 'DELETE',
...
describe('DELETE /api/instructor/assembled-quizzes/[quizId]/excluded-questions/[questionId]', () => {
  ...
  it('re-includes the question, touching only quiz_excluded_question', async () => {
    queueRole('instructor');
    ...
    expect(h.state.deletes).toContainEqual({ table: 'quiz_excluded_question', column: 'assembled_quiz_id', value: 'quiz-1' });
    expect(h.state.deletes).toContainEqual({ table: 'quiz_excluded_question', column: 'question_id', value: 'q-1' });
    expect(h.state.tables).not.toContain('question');
  });

  it('is idempotent: re-including a question that was never excluded is still 200', async () => {
    queueRole('instructor');
    queue('assembled_quiz', { data: quizRow(), error: null });
    queue('quiz_excluded_question', { data: null, error: null }); // delete matches 0 rows, still no error
