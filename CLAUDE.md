...
`loadInstructorActivities` is deliberately **not** cached, and shouldn't be retrofitted: the student-scoped stores hold data changed only by that student's own actions, so the page causing the change can refresh it. The class-wide list changes whenever *any* student answers something, which the instructor's tab never learns about — there is no invalidation point, only staleness.

**Logout clears all eight.** `lib/clientCaches.ts`'s `clearAllClientCaches()` is called from `clearStoredAccessToken` (`lib/authClient.ts`), so both ways a session ends are covered — the explicit `signOut()` and `UserProvider` giving up after a failed refresh. This matters beyond staleness: `instructorStudentsStore` and `acceptanceCriteriaSubmissionsStore` hold *other people's* names, answers and grades. `lib/activityStore.ts` is deliberately excluded — it is the only copy of the student's level/best score/title, so clearing it would be data loss, not cleanup.
