...
- `lib/sessionRules.ts` holds the shared constants (`QUESTIONS_PER_SESSION = 4`, `START_DIFFICULTY_LEVEL = 1`, `PASS_RATIO = 0.75`, `SESSION_COLUMNS`, `SESSION_STATUSES`) so routes cannot drift apart. Partial credit is a known gap: `scoreForAnswer` is all-or-nothing until `answer` gains a score column.
...
Nine stores — `lib/scoreStore.ts`, `completedSessionsStore.ts`, `completedAttemptsStore.ts`, `activityLogStore.ts`, `instructorActivityStore.ts`, `instructorStudentsStore.ts`, `acceptanceCriteriaSubmissionsStore.ts`, `instructorLlmConfigStore.ts`, and `instructorQuestionsStore.ts` — are a *different* kind of localStorage use from `activityStore.ts`: not mocks, but caches in front of real API calls. All share one shape: a versioned key, SSR-guarded read/write, only typed getters/setters exported (never the raw store), keyed by `studentId`/`instructorId` — `completedAttemptsStore` additionally by `activityType`, `instructorLlmConfigStore` by `provider`+`model`. `sessionClient`'s loaders take an optional id to opt into the cache and a `forceRefresh` flag to bypass and re-cache it. Copy this shape for any new cache, and add a `clearCached…()` to it (see below).

The invalidation rule differs: score and completed-sessions only change when a session *completes* (the play page's `handleContinue` refreshes them there), but `activityLogStore` also contains in-progress and abandoned sessions — anything that starts, answers, or abandons must `forceRefresh` it too. `instructorQuestionsStore` needs invalidation on a different trigger again: `createQuestion`/`updateQuestion` (`lib/sessionClient.ts`) write the fresh question list straight back into the cache on every save, since a question the instructor just added or edited must never render stale. `instructorStudentsStore` and `acceptanceCriteriaSubmissionsStore` need no invalidation at all (their own header comments say why).
...
**Logout clears all nine.** `lib/clientCaches.ts`'s `clearAllClientCaches()` is called from `clearStoredAccessToken` (`lib/authClient.ts`), so both ways a session ends are covered — the explicit `signOut()` and `UserProvider` giving up after a failed refresh. This matters beyond staleness: `instructorStudentsStore`, `instructorActivityStore`, and `acceptanceCriteriaSubmissionsStore` hold *other people's* names, answers and grades. `lib/activityStore.ts` is deliberately excluded — it is the only copy of the student's level/best score/title, so clearing it would be data loss, not cleanup.
